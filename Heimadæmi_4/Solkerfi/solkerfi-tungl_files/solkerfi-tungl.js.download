...
function render()
{
    ...
    mv = mult( mv, translate( 1.4, 0.0, 0.0 ) );
	...
	// teikna "gervitungl"
    var mvtt = mult( mvt, rotateX(34.5*rotYear ) );
    mvtt = mult( mvtt, translate( 0, 1.0, 0 ) );
    mvtt = mult( mvtt, scalem( 0.5, 0.5, 0.5 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mvtt));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
