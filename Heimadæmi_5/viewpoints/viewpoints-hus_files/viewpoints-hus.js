...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousemove", function(e){
        if(movement) {
            camXdir	= ( camXdir + (e.offsetX - origX) );
            origX = e.offsetX;
            origY = e.offsetY;
			console.log(camXdir);
        }
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
			...
			case 57:	// 8: from beside the car
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Frá húsi að bílnum";
                break;
            ...
            case 38:    // up arrow
				if (view != 0){
					...
					camXrad = radians(camXdir);
					camXpos += 1*Math.sin(camXrad);
					camYpos += 1*Math.cos(camXrad);
					camYdir += 1;
...
function render()
{
    ...
    switch( view ) {
		case 0:
			...
			mv = lookAt( vec3(camXpos, camYpos, 5), vec3(camXdir, camYpos+90, 0), vec3(0.0, 0.0, 1.0 ) );
			drawScenery( mv );
		...
		case 9:
			// Static viewpoint inside the track; camera follows car
			mv = lookAt( vec3(-40.0, 140.0, 17.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
			drawScenery( mv );
			mv = mult( mv, translate(carXPos, carYPos, 0.0) );
			mv = mult( mv, rotateZ( carDirection ) ) ;
			drawCar( mv );
