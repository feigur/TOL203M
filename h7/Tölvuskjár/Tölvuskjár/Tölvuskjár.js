...
var return_screenX = false;
var return_screenY = false;
...
function configureTexture( img, prog ) {
...
window.onload = function init()
{
    ...
    configureTexture( img, program2 );

    matrixLoc2 = gl.getUniformLocation( program2, "rotation" );
...
function render()
{
	...
	if (spinX > 0 && return_screenX){
		spinX -= 5;
		if (spinX <= 0){
			return_screenX = false;
			spinX = 0;
		}
	}
	else if (spinX < 0 && return_screenX){
		spinX += 5;
		if (spinX >= 0){
			return_screenX = false;
			spinX = 0;
	...
	if (spinY > 0 && return_screenY){
		spinY -= 5;
		if (spinY <= 0){
			return_screenY = false;
			spinY = 0;
		}
	}
	else if (spinY < 0 && return_screenY){
		spinY += 5;
		if (spinY >= 0){
			return_screenY = false;
			spinY = 0;
    ...
    // teikna "sólina"
    mv = mult( mv, scalem( 0.5, 0.05, 0.4 ) );
    ...
    mv = mult( mv, scalem( 0.05, 0.4, 0.1 ) );
	mv = mult( mv, rotateX(270));
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
	...
	mv = mult( mv, rotateZ(rotation));
	mv = mult( mv, rotateX(180));
    mv = mult( mv, scalem( 0.8, 0.6, 0.2 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
